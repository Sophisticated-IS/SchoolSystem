...
internal sealed class GetAllPupilsHandler : IRequestHandler<GetAllPupilsQuery, IEnumerable<Pupil>>
{
    ...
    public async Task<IEnumerable<Pupil>> Handle(GetAllPupilsQuery request, CancellationToken cancellationToken)
    {
        var pupils =  await _schoolDbContext.Pupils.ToListAsync(cancellationToken: cancellationToken).ConfigureAwait(false);
        return pupils.Select(p=>_mapper.Map<Pupil>(p));  
    }
}